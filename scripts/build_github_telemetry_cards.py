...
FALLBACK_REPOS = [
    ...
    {
        "name": "hybrid-quantum-classical-gan-research",
        "language": "Python",
        "stargazers_count": 0,
        "forks_count": 0,
        "pushed_at": "2026-06-10T00:00:00Z",
...
def ellipsize(value: str, limit: int) -> str:
    """Truncate text to a stable SVG display width."""
    value = str(value)
    if len(value) <= limit:
        return value
...
def github_headers() -> dict[str, str]:
    """Build GitHub API headers from the optional access token."""
    headers = {
...
def fetch_repos() -> tuple[dict[str, object], list[dict[str, object]], dict[str, int], bool]:
    """Fetch profile, repository, language-volume, and contribution metadata."""
    try:
...
def parse_time(value: object) -> datetime | None:
    """Parse an ISO GitHub timestamp into an aware datetime."""
    if not value:
        return None
    try:
...
def svg_open(width: int, height: int, title: str, desc: str) -> list[str]:
    """Build accessible opening markup and shared styles for a telemetry SVG."""
    return [
...
def svg_close(parts: list[str], path: Path) -> None:
    """Close and write an SVG document assembled from markup parts."""
    parts.append("</svg>\n")
    path.write_text("\n".join(parts), encoding="utf-8")


def metric_box(x: int, y: int, w: int, label: str, value: object, accent: str) -> str:
    """Build one labelled telemetry metric box."""
    return "\n".join(
        [
...
def bar_rows(items: list[tuple[str, int]], x: int, y: int, w: int, row_h: int = 28) -> list[str]:
    """Build proportional horizontal bar rows for integer metrics."""
    parts: list[str] = []
    ...
    for idx, (label, value) in enumerate(items):
        ...
        parts.append(
            f'<rect x="{bar_x}" y="{yy}" width="{bar_w}" height="14" rx="7" fill="{color}"/>'
        )
...
def percent_rows(
    items: list[tuple[str, int]], x: int, y: int, w: int, row_h: int = 30
) -> list[str]:
    """Build labelled percentage rows normalized to their total."""
    ...
    for idx, (label, value) in enumerate(items):
        ...
        parts.append(
            f'<rect x="{bar_x}" y="{yy}" width="{max(6, int(pct * w))}" height="16" rx="8" fill="{color}"/>'
        )
        parts.append(
            f'<text x="{bar_x + w + 12}" y="{yy + 13}" class="mono">{pct * 100:4.1f}%</text>'
        )
    return parts


def build_intro() -> None:
    """Generate the GitHub telemetry introduction card."""
    parts = [
...
def build_profile_details(
    ...
    parts = svg_open(
        ...
        "Local checked-in telemetry snapshot for fishman7337.",
    )
    parts.append(
        '<text x="28" y="78" class="subtitle">Local SVG snapshot - no third-party card service at README render time</text>'
    )
    metric_y = 102
    ...
    for idx, repo in enumerate(recent):
        ...
        color = PALETTE[idx % len(PALETTE)]
        parts.append(
            f'<rect x="520" y="{y - 18}" width="392" height="34" rx="14" fill="{COLORS["panel"]}" stroke="{color}" stroke-opacity="0.30"/>'
        )
        parts.append(f'<circle cx="534" cy="{y - 5}" r="5" fill="{color}"/>')
        parts.append(
            f'<text x="548" y="{y}" class="body">{esc(ellipsize(str(repo.get("name")), 40))}</text>'
        )
        parts.append(f'<text x="548" y="{y + 14}" class="mono">{esc(when)} SGT</text>')
    source = "live GitHub API" if live else "offline fallback"
    parts.append(
        f'<text x="28" y="418" class="mono">Generated {esc(generated)} from {esc(source)}. Checked into this repository for reliable README rendering.</text>'
    )
...
def build_stats(profile: dict[str, object], repos: list[dict[str, object]], generated: str) -> None:
    """Generate aggregate GitHub account statistics."""
    ...
    parts.append(
        metric_box(
            28, 84, 190, "Public repos", profile.get("public_repos", len(repos)), COLORS["cyan"]
        )
    )
    ...
    parts.append(
        metric_box(
            ...
            190,
            "Stars",
            sum(int(r.get("stargazers_count") or 0) for r in repos),
            COLORS["purple"],
        )
    )
    parts.append(
        metric_box(
            ...
            190,
            "Forks",
            sum(int(r.get("forks_count") or 0) for r in repos),
            COLORS["pink"],
        )
    )
...
def build_productive_time(repos: list[dict[str, object]], generated: str) -> None:
    """Generate the repository activity-timing card."""
    ...
    parts = svg_open(
        ...
        280,
        "Update Time by Repo",
        "Repository pushed_at timestamps grouped by Singapore time.",
    )
    parts.append(
        '<text x="28" y="78" class="subtitle">Default-branch update hour, Singapore time</text>'
    )
...
def build_repos_per_language(repos: list[dict[str, object]], generated: str) -> None:
    """Generate repository counts grouped by primary language."""
...
def build_language_volume(language_bytes: dict[str, int], generated: str) -> None:
    """Generate language-byte distribution telemetry."""
    items = sorted(language_bytes.items(), key=lambda item: item[1], reverse=True)[:5]
    parts = svg_open(
        470, 280, "Top Languages by Code", "Language bytes across public repositories."
    )
...
def build_activity_snapshot(repos: list[dict[str, object]], generated: str) -> None:
    ...
    parts = svg_open(
        ...
        "Recent public repository updates served as a local SVG.",
    )
    parts.append(
        '<text x="28" y="78" class="subtitle">Recent default-branch updates from public owner repositories</text>'
    )
    for idx, repo in enumerate(recent):
        x = 34
        y = 108 + idx * 29
        ...
        color = PALETTE[idx % len(PALETTE)]
        parts.append(
            f'<rect x="{x}" y="{y - 18}" width="892" height="25" rx="12" fill="{COLORS["panel"]}" stroke="{color}" stroke-opacity="0.35"/>'
        )
        parts.append(f'<circle cx="{x + 16}" cy="{y - 5}" r="5" fill="{color}"/>')
        parts.append(
            f'<text x="{x + 32}" y="{y}" class="body">{esc(ellipsize(str(repo.get("name")), 64))}</text>'
        )
        parts.append(f'<text x="760" y="{y}" class="mono">{esc(when)} SGT</text>')
    parts.append(
        f'<text x="28" y="308" class="mono">Snapshot {esc(generated)} - checked into this repository for reliable README rendering</text>'
    )
    svg_close(parts, OUT / "activity-snapshot.svg")


def main() -> None:
    """Fetch GitHub telemetry and regenerate every telemetry card."""
