...
def svg_open(width: int, height: int, title: str, desc: str) -> list[str]:
    return [
        ...
        "</linearGradient>",
        "<style><![CDATA[",
        ".title{font:800 28px Segoe UI,Inter,Arial,sans-serif;fill:#E5E7EB}",
        ".subtitle{font:700 14px Consolas,Menlo,monospace;fill:#22D3EE}",
        ".label{font:700 13px Consolas,Menlo,monospace;fill:#94A3B8}",
        ".value{font:800 25px Segoe UI,Inter,Arial,sans-serif;fill:#E5E7EB}",
        ".body{font:600 14px Segoe UI,Inter,Arial,sans-serif;fill:#CBD5E1}",
        ".mono{font:600 12px Consolas,Menlo,monospace;fill:#94A3B8}",
        ...
        f"<path d=\"M28 54 H{width - 28}\" stroke=\"url(#neon)\" stroke-width=\"2\" opacity=\"0.7\"/>",
...
def bar_rows(items: list[tuple[str, int]], x: int, y: int, w: int, row_h: int = 27) -> list[str]:
    parts: list[str] = []
    ...
    for idx, (label, value) in enumerate(items):
        ...
        parts.append(f"<text x=\"{x}\" y=\"{yy + 14}\" class=\"body\">{esc(ellipsize(label, 22))}</text>")
        parts.append(f"<rect x=\"{x + 170}\" y=\"{yy}\" width=\"{w}\" height=\"14\" rx=\"7\" fill=\"#0F172A\"/>")
        parts.append(f"<rect x=\"{x + 170}\" y=\"{yy}\" width=\"{bar_w}\" height=\"14\" rx=\"7\" fill=\"{color}\"/>")
        parts.append(f"<text x=\"{x + 180 + w}\" y=\"{yy + 12}\" class=\"mono\">{value}</text>")
    return parts
...
def percent_rows(items: list[tuple[str, int]], x: int, y: int, w: int, row_h: int = 30) -> list[str]:
    ...
    for idx, (label, value) in enumerate(items):
        ...
        pct = value / total
        color = PALETTE[idx % len(PALETTE)]
        parts.append(f"<text x=\"{x}\" y=\"{yy + 15}\" class=\"body\">{esc(ellipsize(label, 20))}</text>")
        parts.append(f"<rect x=\"{x + 165}\" y=\"{yy}\" width=\"{w}\" height=\"16\" rx=\"8\" fill=\"#0F172A\"/>")
        parts.append(f"<rect x=\"{x + 165}\" y=\"{yy}\" width=\"{max(6, int(pct * w))}\" height=\"16\" rx=\"8\" fill=\"{color}\"/>")
        parts.append(f"<text x=\"{x + 176 + w}\" y=\"{yy + 13}\" class=\"mono\">{pct * 100:4.1f}%</text>")
    return parts
...
def build_profile_details(profile: dict[str, object], repos: list[dict[str, object]], language_bytes: dict[str, int], generated: str, live: bool) -> None:
    ...
    parts = svg_open(960, 300, "GitHub Telemetry Snapshot", "Local checked-in telemetry snapshot for fishman7337.")
    parts.append(f"<text x=\"28\" y=\"78\" class=\"subtitle\">Local SVG snapshot - no third-party card service at README render time</text>")
    parts.append(metric_box(28, 104, 150, "Repos", profile.get("public_repos", len(repos)), COLORS["cyan"]))
    parts.append(metric_box(194, 104, 150, "Stars", sum(int(r.get("stargazers_count") or 0) for r in repos), COLORS["purple"]))
    parts.append(metric_box(360, 104, 150, "Forks", sum(int(r.get("forks_count") or 0) for r in repos), COLORS["green"]))
    parts.append(metric_box(526, 104, 150, "Followers", profile.get("followers", 0), COLORS["pink"]))
    parts.append(metric_box(692, 104, 150, "Timezone", "SGT", COLORS["amber"]))
    parts.append("<text x=\"28\" y=\"214\" class=\"subtitle\">Top language footprint</text>")
    parts.extend(percent_rows([(k, int(v)) for k, v in top_languages], 28, 234, 300, row_h=22))
    parts.append("<text x=\"520\" y=\"214\" class=\"subtitle\">Recently updated repos</text>")
    for idx, repo in enumerate(recent):
        dt = parse_time(repo.get("pushed_at"))
        when = dt.astimezone(SGT).strftime("%Y-%m-%d") if dt else "unknown"
        y = 238 + idx * 22
        parts.append(f"<circle cx=\"530\" cy=\"{y - 4}\" r=\"5\" fill=\"{PALETTE[idx % len(PALETTE)]}\"/>")
        parts.append(f"<text x=\"546\" y=\"{y}\" class=\"body\">{esc(ellipsize(str(repo.get('name')), 34))}</text>")
        parts.append(f"<text x=\"820\" y=\"{y}\" class=\"mono\">{esc(when)}</text>")
    source = "live GitHub API" if live else "offline fallback"
    parts.append(f"<text x=\"692\" y=\"286\" class=\"mono\">Generated {esc(generated)} from {esc(source)}</text>")
...
def build_stats(profile: dict[str, object], repos: list[dict[str, object]], generated: str) -> None:
    parts = svg_open(470, 240, "Repository Stats", "Repository count, stars, forks, and recency.")
    active_90 = 0
    ...
    for repo in repos:
        ...
        if dt and (now - dt).days <= 90:
            active_90 += 1
    parts.append(metric_box(28, 78, 190, "Public repos", profile.get("public_repos", len(repos)), COLORS["cyan"]))
    parts.append(metric_box(252, 78, 190, "Updated in 90d", active_90, COLORS["green"]))
    parts.append(metric_box(28, 164, 190, "Stars", sum(int(r.get("stargazers_count") or 0) for r in repos), COLORS["purple"]))
    parts.append(metric_box(252, 164, 190, "Forks", sum(int(r.get("forks_count") or 0) for r in repos), COLORS["pink"]))
    parts.append(f"<text x=\"28\" y=\"226\" class=\"mono\">Snapshot {esc(generated)} UTC+08</text>")
...
def build_productive_time(repos: list[dict[str, object]], generated: str) -> None:
    ...
    parts = svg_open(470, 240, "Update Time by Repo", "Repository pushed_at timestamps grouped by Singapore time.")
    parts.append("<text x=\"28\" y=\"76\" class=\"subtitle\">Default-branch update hour, Singapore time</text>")
    chart_x, chart_y, chart_w, chart_h = 42, 96, 392, 100
    ...
    for hour in [0, 6, 12, 18, 23]:
        x = chart_x + hour * (bar_w + gap)
        parts.append(f"<text x=\"{x}\" y=\"218\" class=\"mono\">{hour}</text>")
    parts.append(f"<text x=\"300\" y=\"226\" class=\"mono\">Snapshot {esc(generated)}</text>")
...
def build_repos_per_language(repos: list[dict[str, object]], generated: str) -> None:
    ...
    parts = svg_open(470, 240, "Top Languages by Repo", "Repository primary languages.")
    parts.extend(bar_rows([(k, int(v)) for k, v in items], 28, 82, 200))
    parts.append(f"<text x=\"300\" y=\"226\" class=\"mono\">Snapshot {esc(generated)}</text>")
...
def build_language_volume(language_bytes: dict[str, int], generated: str) -> None:
    ...
    parts = svg_open(470, 240, "Top Languages by Code", "Language bytes across public repositories.")
    parts.extend(percent_rows([(k, int(v)) for k, v in items], 28, 82, 200))
    parts.append(f"<text x=\"276\" y=\"226\" class=\"mono\">Snapshot {esc(generated)}</text>")
    svg_close(parts, OUT / "language-volume.svg")


def build_activity_snapshot(repos: list[dict[str, object]], generated: str) -> None:
    recent = sorted(repos, key=lambda repo: str(repo.get("pushed_at") or ""), reverse=True)[:8]
    parts = svg_open(960, 260, "Recent Repository Activity", "Recent public repository updates served as a local SVG.")
    ...
    for idx, repo in enumerate(recent):
        ...
        color = PALETTE[idx % len(PALETTE)]
        parts.append(f"<rect x=\"{x}\" y=\"{y - 18}\" width=\"410\" height=\"26\" rx=\"13\" fill=\"{COLORS['panel']}\" stroke=\"{color}\" stroke-opacity=\"0.35\"/>")
        parts.append(f"<circle cx=\"{x + 16}\" cy=\"{y - 5}\" r=\"5\" fill=\"{color}\"/>")
        parts.append(f"<text x=\"{x + 30}\" y=\"{y}\" class=\"body\">{esc(ellipsize(str(repo.get('name')), 31))}</text>")
        parts.append(f"<text x=\"{x + 306}\" y=\"{y}\" class=\"mono\">{esc(when)} SGT</text>")
    parts.append(f"<text x=\"28\" y=\"238\" class=\"mono\">Snapshot {esc(generated)} - checked into this repository for reliable README rendering</text>")
